...
void onWebSocketMessage(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len)
{
    if (type == WS_EVT_DATA)
    {
        ...
        Serial.printf("Nachricht vom Client 123: %s\n", message.c_str());
        JsonDocument doc;
        ...
        if (doc["eventName"] == "MOVE_DOWN")
        {
            motor1.down();
        }

        if (doc["eventName"] == "MOVE_UP")
        {
            motor1.up();
        }

        if (doc["eventName"] == "MOVE_STOP")
        {
            motor1.stop();
        }

        // Sende eine Antwort zurück
        // client->text("Nachricht empfangen: " + message);
    }
    if (type == WS_EVT_CONNECT)
    {
        ...
        doc["payload"]["position"] = motor1.getCounterValue(); // Beispielwert
...
void ws_loop()
{
    ...
    int newMotor1Value = motor1.getCounterValue();
